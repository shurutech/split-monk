...
const NAV_ITEMS = [
  ...
  { href: '/groups/new', icon: PlusCircle,       label: 'New Trip' },
...
export function BottomNav() {
  ...
  function isActive(href: string) {
    // Home is active on dashboard and all group detail pages
    if (href === '/dashboard') {
      return pathname === '/dashboard' || (pathname.startsWith('/groups/') && pathname !== '/groups/new')
    }
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center gap-1 px-4 py-2 transition-colors duration-150 ${
                active ? 'text-[#7C6BF8]' : 'text-faint hover:text-[#8E8E9A]'
              }`}
            >
