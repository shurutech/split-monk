    ...
    <div className="space-y-5">
      ...
      {myBalance && (
        <div className={`rounded-md border ${
          ...
          {/* ── Paid for you — by person (shown in all states) ── */}
                                ...
                                return (
                                  <div key={e.id} className="flex items-center gap-2 px-4 py-2.5">
                                    <div className="flex-1 min-w-0">
                                      ...
                                      <p className="text-faint text-[10px] mt-0.5">{name(uid)} paid {formatINR(e.amount)} · {date}</p>
                                    </div>
      ...
      {settlements.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-[#8E8E9A] text-xs uppercase tracking-wide">
              Suggested settlements
            </h3>
            {isOrganizer && (
              <div className="flex items-center gap-1.5">
                ...
                {showReminders &&
                  settlements.some((s) => userCache[s.from]?.email) && (
                    <button
          ...
          <div className="space-y-3">
            {settlements.map((s) => {
              ...
              const receiverUpi = !isPendingKey(s.to)
                ? userCache[s.to]?.upiId
                : undefined;

              return (
                <div
                  key={key}
                  className="rounded-md border border-[#2A2A32] bg-[#111113] overflow-hidden"
                >
                  {/* From → amount → to */}
                  <div className="flex items-center gap-2 p-4 pb-3 min-w-0">
                    <div className="flex items-center gap-1.5 min-w-0 flex-1">
                      {avatar(s.from, 28)}
                      <span className="text-[#F2F2F7] text-sm font-medium truncate">
                        {name(s.from)}
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                      <ArrowRight size={13} className="text-faint" />
                      <span className="font-mono text-[#F2F2F7] text-sm font-semibold">
                        {formatINR(s.amount)}
                      </span>
                      <ArrowRight size={13} className="text-faint" />
                    </div>
                    <div className="flex items-center gap-1.5 min-w-0 flex-1 justify-end">
                      <span className="text-[#F2F2F7] text-sm font-medium truncate text-right">
                        {name(s.to)}
                      </span>
                      {avatar(s.to, 28)}
                    </div>
                  ...
                  {showReminders && userCache[s.from]?.email && (
                    <div className="border-t border-[#1A1A1F] px-4 py-2.5 flex items-center justify-between">
                      <p className="text-faint text-[11px]">
                        ...
                        <span className="text-[#F2F2F7]">{name(s.from)}</span>
                      </p>
                      <button
      ...
      {/* ── All balances ───────────────────────────────────────────────────── */}
      <div>
        <h3 className="text-[#8E8E9A] text-xs uppercase tracking-wide mb-3">
          All balances
        </h3>
        <div className="space-y-2">
          {balances.map((b) => (
            <div
              key={b.uid}
              className="flex items-center gap-3 py-2.5 px-3 rounded-sm border border-[#2A2A32] bg-[#111113]"
            >
              {avatar(b.uid, 32)}
              ...
              {isPendingKey(b.uid) && (
                <span className="text-warning text-[10px] font-medium mr-1 shrink-0">
                  invited
                </span>
              )}
              <span
                className={`font-mono text-sm font-medium shrink-0 ${Math.abs(b.net) >= 100 ? (b.net > 0 ? "text-success" : "text-[#F87171]") : "text-faint"}`}
              >
                {Math.abs(b.net) < 100
                  ? "Settled"
                  : b.net > 0
            ...
            {recordedSettlements.map((s) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {s.note && (
                    <p className="text-faint text-[10px] truncate mt-0.5">
                      {s.note}
                    </p>
                  )}
                </div>
                <div className="text-right shrink-0">
                  <p className="font-mono text-xs text-success font-medium">
                    {formatINR(s.amount)}
                  </p>
